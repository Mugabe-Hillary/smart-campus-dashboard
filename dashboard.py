...
def check_password():
    ...
    password = os.environ.get("DASHBOARD_PASSWORD", "cisco1234")  # Default password for demo
    ...
    if "password_correct" not in st.session_state:
        ...
        st.text_input("Password", type="password", on_change=password_entered, key="password", placeholder="Enter dashboard password")
        st.info("Default password: admin123 (change via DASHBOARD_PASSWORD env variable)")
        return False
...
def create_environmental_chart(df):
    ...
    if 'humidity' in df.columns:
        fig.add_trace(
            go.Scatter(x=df.index, y=df['humidity'], 
...
with st.sidebar:
    ...
    selected_range = st.selectbox(
        ...
        index=list(time_ranges.values()).index(st.session_state.time_range) if st.session_state.time_range in time_ranges.values() else 1
    )
    ...
    with st.form("ml_form"):
        raw_temp = st.number_input("Temperature (°C)", value=25.0, step=0.1, min_value=-40.0, max_value=85.0)
        raw_hum = st.number_input("Humidity (%)", value=60.0, step=0.1, min_value=0.0, max_value=100.0)
...
if auto_refresh and refresh_rate:
    refresh_interval = st_autorefresh(interval=refresh_rate * 1000, limit=None, key="dashboard_refresh")
...
with col1:
    temp_val = latest_env['temperature'] if latest_env is not None and 'temperature' in latest_env else "N/A"
    temp_status, temp_icon = evaluate_status(temp_val, temp_thresholds)
    st.metric("🌡️ Temperature", f"{temp_val}°C" if temp_val != "N/A" else "N/A", delta=None)
    st.write(f"{temp_icon} Status")

with col2:
    hum_val = latest_env['humidity'] if latest_env is not None and 'humidity' in latest_env else "N/A"
...
with col3:
    light_val = latest_class['light_level'] if latest_class is not None and 'light_level' in latest_class else "N/A"
...
with col4:
    motion_detected = latest_sec is not None and latest_sec.get('motion_detected', False) if latest_sec is not None else False
...
tab1, tab2, tab3, tab4 = st.tabs(["🌿 Environment", "📚 Classroom", "🔒 Security", "📋 System Health"])
...
with tab1:
    ...
    with col2:
        if not df_env.empty:
            ...
            if 'temperature' in df_env.columns:
                ...
                st.metric("Min/Max", f"{df_env['temperature'].min():.1f}°C / {df_env['temperature'].max():.1f}°C")
    ...
    if not df_env.empty:
        with st.expander("📋 Raw Data", expanded=False):
            st.dataframe(df_env.tail(50).sort_index(ascending=False), use_container_width=True)

with tab2:
    if not df_class.empty and not class_error:
        if 'light_level' in df_class.columns:
            fig_light = px.line(df_class, x=df_class.index, y='light_level', 
        ...
        with st.expander("📋 Classroom Data", expanded=False):
            st.dataframe(df_class.tail(50).sort_index(ascending=False), use_container_width=True)
    else:
...
with tab3:
    if not df_sec.empty and not sec_error:
        ...
        cols_to_show = [col for col in ['rfid_uid', 'access_status', 'motion_detected', 'distance_cm'] 
            ...
            def highlight_security(row):
                ...
                for col in row.index:
                    if col == 'access_status':
                        colors.append('background-color: lightgreen' if row[col] else 'background-color: lightcoral')
                    elif col == 'motion_detected':
                        colors.append('background-color: yellow' if row[col] else 'background-color: lightblue')
                    else:
                        colors.append('')
                return colors
            
            st.dataframe(sec_display.style.apply(highlight_security, axis=1), use_container_width=True)
        else:
...
with tab4:
    ...
    with col2:
        data_freshness = "🟢 Fresh" if any([not df_env.empty, not df_class.empty, not df_sec.empty]) else "🔴 Stale"
